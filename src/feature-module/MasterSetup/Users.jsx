...
import { deleteUser, getUsers } from "../../core/redux/action";
...
const Users = () => {
  ...
  const [search, setSearch] = useState({ name: "Choose User", date: null, status: true });
  const [option, setOption] = useState([{ value: "Choose User", label: 'Choose User' }]);
  ...
  const handleFilter = () => {
    if (search.name === "Choose Username" && search.date === null) {
      setDataSource(postData.filter((x) => x.isActive === search.status));
    }
    else if (search.name != "Choose Username" && search.date === null) {
      setDataSource(postData.filter((x) => x.username.toLowerCase().includes(search.name.toLowerCase()) && x.isActive === search.status));
    }
    else if (search.name === "Choose Username" && search.date != null) {
      setDataSource(postData.filter((x) => format(x.createdDate, "yyyy-MM-dd") === search.date && x.isActive === search.status));
    }
    else {
      setDataSource(postData.filter((x) => x.username.toLowerCase().includes(search.name.toLowerCase()) && format(x.createdDate, "yyyy-MM-dd") === search.date && x.isActive === search.status));
    }
  }
  ...
  return (
    <div>
      ...
      <AddUser userId={loginUser?.userId} insertMode={insertMode} setInsertMode={setInsertMode} />
      <EditUser id={getInvId} isEditMode={getEditMode} setEditMode={setEditMode} />
    </div>
  );
};
